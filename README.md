...
Serve config is re-applied every boot (`serve reset` then SSH TCP 22 and/or HTTPS 443 from current env), so toggling `TAILSCALE_HTTPS` off clears the HTTPS handler without leaving stale config. SSH and HTTPS share that reset — both are rebuilt together.
